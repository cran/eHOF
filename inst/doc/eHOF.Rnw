...
\end{center}
\caption{
Scatter Plot of optima (mean groundwater level below surface in centimetre in the Elbaue dataset) with best HOF model versus response models restricted to be unimodal symmetric ('IV'). The cross is plotted at x=-270 but in fact there is no x values since it is recognized to be a model I (no response) species by AICc selection. The green lines are representations of species without punctual optimum.
 }
\label{fig:compOPT}
\end{figure}
