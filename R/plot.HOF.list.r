...
fitfun <- function(x, test, modeltypes,...) fitted(x, model = pick.model(x, test = test, modeltypes=modeltypes, gam=FALSE, quiet = TRUE, ...))/x$M

 if (plottype == "layout") {
	M <- mods[[1]]$M
	minresp <- if(missing(yl)) 0 else yl[1]
        maxresp <- 
	if(missing(yl)) max(sapply(mods, function(x) max(x$models[[pick.model(x, test = test, modeltypes, gam=FALSE, quiet = TRUE)]]$fitted )/M))  else yl[2]
 ...
 if (plottype == "all") {
        m <- max(sapply(mods, function(x) fitted(x, model=pick.model(x, gam=FALSE, quiet = TRUE, ...)))/mods[[1]]$M)
        grad <- mods[[1]]$x
        plot(0, xlim = c(min(grad), max(grad)), ylim=c(0,m), ylab = "Predicted probability", xlab = xlabel, type = "n", ...)
        at <- order(grad)
...
#            par(xpd = TRUE)
        ...
        legtext <- paste(names(mods), "(", sapply(mods, function(x, ...) pick.model(x,modeltypes, gam=FALSE, quiet = TRUE, ...)), ")")
